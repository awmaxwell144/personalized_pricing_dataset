...
## Count Term Matches
- Run `python3 utilities/count_terms.py <company>` from the repository root.
- Example: `python3 utilities/count_terms.py delta`
- This reads regexes from `utilities/term_list.txt` and writes two CSVs in the company directory:
  - `<company>_raw_term_counts.csv` for full `raw.txt` source counts.
  - `<company>_statement_term_counts.csv` for `extracted_company_statement.txt` counts.
